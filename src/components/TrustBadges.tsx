...
const TrustBadges = () => {
  return (
    <section className="bg-xtreme-dark text-white py-20 md:py-24 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 max-w-6xl mx-auto">
          {badges.map(({ icon: Icon, title, desc }, i) => (
            <div key={i} className="text-center group">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-accent/15 text-accent mb-6 group-hover:bg-accent group-hover:text-accent-foreground transition-all duration-500">
                <Icon className="w-7 h-7" />
              </div>
              <h3 className="font-playfair text-2xl md:text-3xl font-semibold mb-3 leading-tight">
                {title}
              </h3>
