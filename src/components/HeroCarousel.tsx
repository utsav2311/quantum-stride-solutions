...
const HeroCarousel = () => {
  ...
  return (
    <section className="relative h-screen overflow-hidden">
        ...
        <div className="embla__container flex">
          {slides.map((slide, index) => (
            <div key={index} className="embla__slide flex-none w-full relative">
              ...
              <div className="relative z-10 h-full flex items-center justify-center px-16 md:px-24 lg:px-32 xl:px-40 2xl:px-48 py-16 md:py-24 lg:py-32 xl:py-40">
                  ...
                  <div className="w-full">
                    <h1 className="text-4xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white mb-8 leading-tight animate-fade-in font-playfair">
                      {slide.title.split(' ').map((word, i) => (
                        <span key={i} className={i === 1 || i === 2 ? "text-accent-foreground bg-accent/20 px-3 rounded-lg" : ""}>
                          {word}{' '}
                        </span>
                      ))}
                    </h1>
      ...
      {/* Scroll Indicator */}
      <div className="absolute bottom-16 left-1/2 transform -translate-x-1/2 text-white/60 animate-bounce cursor-pointer z-20"
